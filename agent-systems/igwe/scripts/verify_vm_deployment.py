    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Messages Dashboard" in response.text or "conversation" in response.text.lower():
                print("✅ Dashboard has expected content")
                return True
            else:
                print("⚠️ Dashboard loaded but content looks unexpected")
                return True
        else:
...
def check_conversation_link():
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Conversation link works (no 404)")
            print("✅ Notification emails will have working links")
            return True
        else:
            print(f"❌ Conversation link returned status {response.status_code}")
            return False
    except Exception as e:
        print(f"❌ Conversation link not accessible: {e}")
        return False
...
def check_inbound_webhook():
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Inbound webhook is working")
            return True
        else:
            print("❌ Inbound webhook returned non-200")
            return False
    except Exception as e:
...
def check_app_base_url():
    ...
    if not app_base_url:
        print("❌ APP_BASE_URL is not set in .env")
    ...
    if "localhost" in app_base_url:
        print("⚠️ APP_BASE_URL is set to localhost")
        print("   This should be your production domain")
        return False
    
    if "reimaginewealth.org" in app_base_url:
        print("✅ APP_BASE_URL is correctly set")
        return True
    
    print("⚠️ APP_BASE_URL might be incorrect")
    return True
