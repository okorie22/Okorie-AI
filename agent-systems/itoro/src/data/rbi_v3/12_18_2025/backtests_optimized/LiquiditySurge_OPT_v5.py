...
class LiquiditySurge(Strategy):
    ...
    atr_period = 10  # Shorter ATR period for more responsiveness
    rsi_period = 10  # Shorter RSI period
    rsi_overbought = 75  # Higher overbought threshold
    rsi_oversold = 25  # Lower oversold threshold
    ema_fast = 20  # Fast EMA for trend filter
    ema_slow = 50  # Slow EMA for trend filter
    min_volume_multiplier = 1.5  # Minimum volume spike multiplier
    ...
    def next(self):
        ...
        if current_bar < max(self.ema_slow, self.atr_period, self.rsi_period):
            return
        ...
        if self.equity > 0 and stop_distance > 0:
            ...
            volatility_factor = max(0.5, min(2.0, atr_value / current_close * 100))
        ...
        if self.position:
            ...
            if self.position_direction == 1:
                if current_close >= self.entry_price + (self.risk_reward_ratio * stop_distance * self.trailing_stop_activation):
                    self.trailing_stop_active = True
                
                if self.trailing_stop_active:
                    trailing_stop_level = current_close - (atr_value * self.trailing_stop_distance)
                    if current_close <= trailing_stop_level:
                        print(f"[EXIT] Trailing stop hit for long position at {current_close:.2f}")
                        self.position.close()
                        self.position_direction = 0
                        self.trailing_stop_active = False
                        return
                ...
                if self.trailing_stop_active:
                    trailing_stop_level = current_close + (atr_value * self.trailing_stop_distance)
                    if current_close >= trailing_stop_level:
                        print(f"[EXIT] Trailing stop hit for short position at {current_close:.2f}")
                        self.position.close()
                        self.position_direction = 0
                        self.trailing_stop_active = False
                        return
            ...
            if bars_held >= self.max_holding_bars:
                print(f"[EXIT] Time-based exit after {bars_held} bars, PnL: {current_pnl:.2f}%")
                self.position.close()
                self.position_direction = 0
                self.trailing_stop_active = False
                return
            ...
            # Trend reversal exit
            if self.position_direction == 1 and ema_fast < ema_slow:
                print("[EXIT] Trend reversal exit for long position")
                self.position.close()
                self.position_direction = 0
                self.trailing_stop_active = False
                return
            elif self.position_direction == -1 and ema_fast > ema_slow:
                print("[EXIT] Trend reversal exit for short position")
                self.position.close()
                self.position_direction = 0
                self.trailing_stop_active = False
                return
        ...
        if not self.position:
            ...
            if volume_sma > 0 and current_volume < volume_sma * self.min_volume_multiplier:
                return
            ...
            if liquidity_change < -self.liquidity_threshold:
                # RSI confirmation (oversold) AND trend filter
                if rsi_value < self.rsi_oversold and long_trend_ok:
                    print(f"[LONG] Entry signal: Liquidity change {liquidity_change:.2f}%, RSI {rsi_value:.2f}, Trend: Bullish")
            ...
            # Short entry: liquidity spike > +50% (euphoric buying)
                ...
                # RSI confirmation (overbought) AND trend filter
                if rsi_value > self.rsi_overbought and short_trend_ok:
                    print(f"[SHORT] Entry signal: Liquidity change {liquidity_change:.2f}%, RSI {rsi_value:.2f}, Trend: Bearish")
