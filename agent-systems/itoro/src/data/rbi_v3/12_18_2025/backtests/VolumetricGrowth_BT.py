...
class VolumetricGrowth(Strategy):
    ...
    def init(self):
        ...
        # We'll need to parse the JSON structure
        ...
        # Volume indicators
        self.volume_ma = self.I(talib.SMA, self.transaction_volume, timeperiod=self.volume_ma_period)
        self.volume_momentum = self.I(talib.ROC, self.transaction_volume, timeperiod=self.momentum_period)
        
        # Count indicators
        self.count_ma = self.I(talib.SMA, self.transaction_count, timeperiod=self.count_ma_period)
        self.count_momentum = self.I(talib.ROC, self.transaction_count, timeperiod=self.momentum_period)
        
        # Calculate thresholds
        self.volume_30d_avg = self.I(talib.SMA, self.transaction_volume, timeperiod=30)
        self.count_30d_avg = self.I(talib.SMA, self.transaction_count, timeperiod=30)
        self.volume_90d_avg = self.I(talib.SMA, self.transaction_volume, timeperiod=90)
        self.count_90d_avg = self.I(talib.SMA, self.transaction_count, timeperiod=90)
        ...
        self.volume_peak = self.I(talib.MAX, self.transaction_volume, timeperiod=20)
        self.count_peak = self.I(talib.MAX, self.transaction_count, timeperiod=20)
        
        # Track trend confirmation
        self.volume_growing = self.I(lambda vol, ma: vol > ma, self.transaction_volume, self.volume_ma)
        self.count_growing = self.I(lambda cnt, ma: cnt > ma, self.transaction_count, self.count_ma)
        
        # Track momentum direction
        self.volume_momentum_rising = self.I(lambda mom: mom > 0, self.volume_momentum)
        self.count_momentum_rising = self.I(lambda mom: mom > 0, self.count_momentum)
        
        # Debug prints
        print("[INFO] Strategy initialized with onchain data")
        print(f"[INFO] Transaction volume range: {self.transaction_volume.min():.2f} to {self.transaction_volume.max():.2f}")
        print(f"[INFO] Transaction count range: {self.transaction_count.min():.2f} to {self.transaction_count.max():.2f}")
    ...
    def next(self):
        ...
        # Get current values
        current_volume = self.transaction_volume[current_idx]
        current_count = self.transaction_count[current_idx]
        ...
        if self.position:
            ...
            if current_idx >= self.confirmation_periods:
                ...
                for i in range(self.confirmation_periods):
                    idx = current_idx - i
                    if not (self.transaction_volume[idx] > self.volume_ma[idx] and 
                            self.volume_momentum[idx] > 0):
                        volume_confirmed = False
                    if not (self.transaction_count[idx] > self.count_ma[idx] and 
            ...
            if volume_conditions and count_conditions and confirmation:
                ...
                if position_size > 0:
                    print(f"[BUY] Opening position: size={position_size}, risk={risk_percentage*100:.1f}%")
