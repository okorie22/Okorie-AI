...
class VolumetricGrowth(Strategy):
    ...
    def init(self):
        ...
        self.volume_growing = self.I(lambda vol, ma: vol > ma, volume_array, self.volume_ma)
        self.count_growing = self.I(lambda cnt, ma: cnt > ma, count_array, self.count_ma)
    ...
    def next(self):
        ...
        # Get current values
        current_volume = self.transaction_volume.iloc[current_idx] if hasattr(self.transaction_volume, 'iloc') else self.transaction_volume[current_idx]
        current_count = self.transaction_count.iloc[current_idx] if hasattr(self.transaction_count, 'iloc') else self.transaction_count[current_idx]
        ...
        if self.position:
            ...
            if current_idx >= self.confirmation_periods:
                ...
                for i in range(self.confirmation_periods):
                    ...
                    vol_val = self.transaction_volume.iloc[idx] if hasattr(self.transaction_volume, 'iloc') else self.transaction_volume[idx]
                    cnt_val = self.transaction_count.iloc[idx] if hasattr(self.transaction_count, 'iloc') else self.transaction_count[idx]
