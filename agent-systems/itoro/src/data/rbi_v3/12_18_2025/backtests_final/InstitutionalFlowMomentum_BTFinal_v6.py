...
print(f"[DEBUG] Combined data columns: {list(combined_data.columns)}")
...
class InstitutionalFlowMomentum(Strategy):
    ...
    def init(self):
        ...
        oi_pct_change = np.zeros(len(oi_array))
    ...
    def next(self):
        ...
        # Check for new entry signals (no existing position)
            ...
            if current_oi_pct >= self.oi_threshold:
                ...
                if trend_ok:
                    print(f"[BUY SIGNAL] OI increased {current_oi_pct:.2f}%, price: {current_price:.2f}")
                    ...
                    if position_size > 0:
                        print(f"[BUY EXECUTING] Position size: {position_size} units")
                        self.buy(size=position_size, sl=current_price * (1 - self.stop_loss_pct))
            ...
            # Short signal: OI decrease <= -15%
                ...
                if trend_ok:
                    print(f"[SELL SIGNAL] OI decreased {current_oi_pct:.2f}%, price: {current_price:.2f}")
                    ...
                    if position_size > 0:
                        print(f"[SELL EXECUTING] Position size: {position_size} units")
                        self.sell(size=position_size, sl=current_price * (1 + self.stop_loss_pct))
                        self.trade_start_bar = current_bar
                    else:
                        print(f"[WARNING] Invalid position size: {position_size}")
