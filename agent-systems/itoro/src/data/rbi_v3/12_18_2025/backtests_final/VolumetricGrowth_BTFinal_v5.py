    ...
    def next(self):
        ...
        if self.position:
            ...
            if volume_conditions and count_conditions and confirmation:
                ...
                position_fraction = risk_percentage / stop_distance
                position_fraction = min(position_fraction, 0.95)  # Max 95% of equity
                
                if position_fraction > 0:
                    print(f"[BUY] Opening position: fraction={position_fraction:.4f}, risk={risk_percentage*100:.1f}%, price={self.data.Close[-1]:.2f}")
                    # FIX: Use fraction-based sizing to ensure execution
                    # CRITICAL FIX: Ensure position_fraction is valid and > 0
                    if position_fraction > 0 and position_fraction <= 1:
                        self.buy(size=position_fraction)
                    else:
                        print(f"[ERROR] Invalid position fraction: {position_fraction}")
                else:
                    print(f"[WARNING] Invalid position fraction: {position_fraction}")
            else:
