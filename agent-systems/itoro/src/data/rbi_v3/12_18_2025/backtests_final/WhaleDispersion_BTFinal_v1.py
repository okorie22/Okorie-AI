...
if 'datetime' in price_data.columns:
    ...
    print("[ERROR] No datetime, timestamp or date column found in price data")
...
class WhaleDispersion(Strategy):
    ...
    def init(self):
        ...
        self.whale_conc = self.I(lambda x: x, self.data.df['whale_concentration'])
        
        # Market cap (in dollars)
        self.market_cap = self.I(lambda x: x, self.data.df['market_cap'])
    ...
    def next(self):
        ...
        if np.isnan(whale_ratio) or np.isnan(market_cap_val):
            return
        ...
        # In real implementation, entry would come from another strategy
            ...
            # Check for bullish crossover: SMA20 crosses above SMA50
            if self.sma20[-2] < self.sma50[-2] and self.sma20[-1] > self.sma50[-1] and current_price > self.sma20[-1]:
