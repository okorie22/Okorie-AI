...
# Check available columns and rename properly
print(f"[DEBUG] Price data columns before rename: {list(price_data.columns)}")

# Find the datetime column (could be 'timestamp', 'datetime', 'date', etc.)
datetime_col = None
...
if datetime_col:
    price_data = price_data.rename(columns={datetime_col: 'datetime'})
else:
    # If no datetime column found, assume first column is datetime
    price_data = price_data.rename(columns={price_data.columns[0]: 'datetime'})
...
print(f"[DEBUG] Funding data columns before rename: {list(funding_data.columns)}")

# Find the datetime column in funding data
funding_datetime_col = None
for col in funding_data.columns:
    if 'time' in col.lower() or 'date' in col.lower():
        funding_datetime_col = col
        break

if funding_datetime_col:
    funding_data = funding_data.rename(columns={funding_datetime_col: 'datetime'})
else:
    # If no datetime column found, assume first column is datetime
    funding_data = funding_data.rename(columns={funding_data.columns[0]: 'datetime'})

# Convert and set datetime
funding_data['datetime'] = pd.to_datetime(funding_data['datetime'])
funding_data = funding_data.set_index('datetime')
...
for col in funding_data.columns:
    if 'funding' in col.lower() or 'rate' in col.lower():
        funding_rate_col = col
        break

if funding_rate_col:
    # Resample funding data to match price data frequency (15m)
    funding_data_resampled = funding_data[funding_rate_col].resample('15min').ffill()
else:
    print("[ERROR] No funding rate column found!")
    # Create empty series
    funding_data_resampled = pd.Series(index=price_data.index, dtype=float)
...
print(f"[DEBUG] First few rows of price data:\n{price_data.head()}")
...
class ExtremeRateArbitrage(Strategy):
    ...
    def init(self):
        # Store funding rate as indicator - FIXED: Use proper indicator syntax
        self.funding_rate = self.I(lambda x: x, self.data.df['FundingRate'], name='FundingRate')
        
        # Calculate extremity score - FIXED: must return array of same length
        def calc_extremity(rate_array):
            result = np.zeros_like(rate_array)
            for i in range(len(rate_array)):
                rate = rate_array[i]
                if rate < self.negative_threshold:
                    result[i] = abs(rate - self.negative_threshold)
                elif rate > self.positive_threshold:
                    result[i] = abs(rate - self.positive_threshold)
                else:
                    result[i] = 0
            return result
        ...
        if not self.position:
            ...
            if len(self.funding_rate) >= 2:
                ...
                if current_rate < self.negative_threshold and prev_rate < self.negative_threshold:
                    if units > 0:
                        print(f"[LONG ENTRY] Rate: {current_rate:.6f}, Size: {units} units, Price: {current_price}")
                        self.buy(size=units, sl=current_price * (1 - self.stop_loss_pct))
                        self.entry_time = len(self.data) - 1
                        self.entry_price = current_price
                        self.position_entry_rate = current_rate
                        return  # Added return to prevent multiple entries
                
                # Short entry: funding rate > +0.05%
                elif current_rate > self.positive_threshold and prev_rate > self.positive_threshold:
                    if units > 0:
                        print(f"[SHORT ENTRY] Rate: {current_rate:.6f}, Size: {units} units, Price: {current_price}")
                        self.sell(size=units, sl=current_price * (1 + self.stop_loss_pct))
                        self.entry_time = len(self.data) - 1
                        self.entry_price = current_price
                        self.position_entry_rate = current_rate
                        return  # Added return to prevent multiple entries
