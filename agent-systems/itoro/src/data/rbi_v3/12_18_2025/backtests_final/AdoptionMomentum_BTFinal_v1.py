    ...
    def init(self):
        ...
        for idx, row in onchain_data.iterrows():
            try:
                ...
                # Extract holder count for the first token
        ...
        # Align holder data with price data timestamps
        price_timestamps = pd.Series(self.data.index)
        aligned_holder_counts = []
        aligned_pct_changes = []
        
        for ts in price_timestamps:
            # Find the closest holder data point on or before this timestamp
            available_data = holder_df[holder_df.index <= ts]
            if len(available_data) > 0:
                latest_data = available_data.iloc[-1]
                holder_count = latest_data['holder_count']
            else:
                holder_count = 0
            
            aligned_holder_counts.append(holder_count)
        
        # Convert to numpy array for percentage change calculation
        holder_array = np.array(aligned_holder_counts)
        
        # Calculate percentage change
        pct_change_array = np.zeros_like(holder_array, dtype=float)
        if len(holder_array) > 1:
            pct_change_array[1:] = np.diff(holder_array) / holder_array[:-1]
            pct_change_array[0] = 0
        
        # Use self.I() wrapper for indicator calculation
        self.holder_pct_change = self.I(
            lambda: pct_change_array,
...
if 'timestamp' not in price_data.columns and 'datetime' in price_data.columns:
...
print(f"[DEBUG] Price data index range: {price_data.index[0]} to {price_data.index[-1]}")
