...
class TEMAConvergenceReversal(Strategy):
    ...
    def manage_position(self, current_bar):
        ...
        if len(self.trades) > 0:
            ...
            # Find entry bar index by checking trade timestamps
            entry_bar = None
            for i in range(len(self.data.Close)):
                if i >= len(self.data.Close):
                    break
                # Check if this bar's timestamp matches trade timestamp
                if hasattr(self.trades[-1], 'entry_time'):
                    if self.data.index[i] == self.trades[-1].entry_time:
                        entry_bar = i
                        break
            if entry_bar is None:
                # Fallback: use current bar minus 1 as approximation
                entry_bar = max(0, current_bar - 1)
        else:
