...
class TEMAConvergenceReversal(Strategy):
    ...
    def manage_position(self, current_bar):
        ...
        if len(self.trades) > 0:
            ...
            entry_price = self.position.size * self.data.Close[-1] / abs(self.position.size)
            
        current_price = self.data.Close[-1]
        candles_since_entry = current_bar - self.trades[-1].entry_bar if len(self.trades) > 0 else 0
