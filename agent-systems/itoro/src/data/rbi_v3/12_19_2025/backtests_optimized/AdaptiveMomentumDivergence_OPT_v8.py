...
class AdaptiveMomentumDivergence(Strategy):
    ...
    def init(self):
        ...
        self.macd_mean = self.I(talib.SMA, self.universal_macd, timeperiod=14)
        self.macd_std = self.I(talib.STDDEV, self.universal_macd, timeperiod=14)
        ...
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, timeperiod=10)
        self.plus_di = self.I(talib.PLUS_DI, self.data.High, self.data.Low, self.data.Close, timeperiod=10)
        self.minus_di = self.I(talib.MINUS_DI, self.data.High, self.data.Low, self.data.Close, timeperiod=10)
        self.macd_signal = self.I(talib.EMA, self.universal_macd, timeperiod=9)
        self.macd_histogram = self.universal_macd - self.macd_signal
    ...
    def next(self):
        if self.position:
            ...
            if self.candles_since_entry >= self.min_holding_candles:
                ...
                macd_hist = self.macd_histogram[-1]
                if current_macd >= 0 and macd_hist < 0:
                    self.position.close()
                    self.candles_since_entry = 0
                    self.trailing_stop = 0
                    self.initial_stop = 0
            ...
            if self.candles_since_entry >= 3 and self.scaled_out < self.scale_out_levels:
                ...
                if current_price >= target_price:
                    scale_out_size = self.position.size * 0.4
        ...
        if not self.position:
            ...
            macd_bullish = current_macd > macd_signal and macd_hist > 0
            
            if range_min_val <= current_macd <= range_max_val:
                if volume_condition and trend_condition and rsi_condition and adx_condition and ema_alignment and di_condition and macd_bullish:
                    atr_value = self.atr[-1]
