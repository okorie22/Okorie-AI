...
class AdaptiveMomentumDivergence(Strategy):
    ...
    def init(self):
        ...
        self.supertrend = self.I(self.calculate_supertrend, self.data.High, self.data.Low, self.data.Close, period=10, multiplier=3)
        self.vwap = self.I(self.calculate_vwap, self.data.High, self.data.Low, self.data.Close, self.data.Volume)
        self.macd_line, self.macd_signal, self.macd_hist = self.I(talib.MACD, self.data.Close, fastperiod=12, slowperiod=26, signalperiod=9)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            volume_condition = current_volume > volume_sma_val * 1.3
            trend_condition = current_close > ema_trend_val
            rsi_condition = 40 <= current_rsi <= 60
            adx_condition = adx_val > 30
            ema_alignment = ema_fast_val > ema_slow_val
            supertrend_condition = current_close > supertrend_val
            vwap_condition = current_close > vwap_val
            macd_bullish = macd_hist_val > 0
            
            if range_min_val <= current_macd <= range_max_val:
                if volume_condition and trend_condition and rsi_condition and adx_condition and ema_alignment and supertrend_condition and vwap_condition and macd_bullish:
                    atr_value = self.atr[-1]
                    volatility_adjusted_risk = min(self.risk_per_trade * (15 / atr_value), 0.10)
                    stop_distance = current_close * 0.08
    ...
    def calculate_scaled_roi_target(self):
        ...
        if candles <= 0:
            return 0.40
        if candles <= 10:
            return 0.40 - (candles / 10) * (0.40 - 0.25)
        if candles <= 30:
            return 0.25 - ((candles - 10) / 20) * (0.25 - 0.15)
        if candles <= 60:
            return 0.15 - ((candles - 30) / 30) * (0.15 - 0.08)
        return 0.08
