...
class AdaptiveMomentumDivergence(Strategy):
    ...
    def init(self):
        ...
        self.range_max = self.macd_mean - 0.1 * self.macd_std
        ...
        if not self.position:
            ...
            if range_min_val <= current_macd <= range_max_val:
                if volume_condition and trend_condition and rsi_condition and adx_condition and ema_alignment and di_condition:
                    atr_value = self.atr[-1]
                    volatility_adjusted_risk = min(self.risk_per_trade * (12 / atr_value), 0.12)
    ...
    def calculate_scaled_roi_target(self):
        ...
        if candles <= 0:
            return max(base_target, 0.25)
        if candles <= 10:
            decay_factor = (candles / 10) * 0.15
            return max(base_target, 0.25 - decay_factor)
        if candles <= 30:
            decay_factor = 0.15 + ((candles - 10) / 20) * 0.10
            return max(base_target, 0.10 - decay_factor)
        if candles <= 60:
            decay_factor = 0.25 + ((candles - 30) / 30) * 0.05
            return max(base_target, 0.05 - decay_factor)
        return max(base_target, 0.02)
