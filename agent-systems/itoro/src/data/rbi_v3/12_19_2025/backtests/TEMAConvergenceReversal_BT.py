...
data_4h.columns = ['Timestamp', 'Open', 'High', 'Low', 'Close', 'Volume']
...
class TEMAConvergenceReversal(Strategy):
    ...
    def init(self):
        ...
        # Store exit TEMA pairs for checking
        self.exit_tema_pairs = [
    ...
    def next(self):
        ...
        if current_bar % 10 == 0:
            ...
            if self.debug_counter % 10 == 0:
                ...
                print(f"  TEMA15: {self.tema15[-1]:.2f}, TEMA30: {self.tema30[-1]:.2f}, TEMA45: {self.tema45[-1]:.2f}, TEMA60: {self.tema60[-1]:.2f}")
                cond1 = self.tema30[-1] < self.tema15[-1]
                cond2 = self.tema45[-1] < self.tema30[-1]
                cond3 = self.tema60[-1] < self.tema45[-1]
                print(f"  Condition1 (T30<T15): {cond1}")
                print(f"  Condition2 (T45<T30): {cond2}")
                print(f"  Condition3 (T60<T45): {cond3}")
    ...
    def check_entry(self, current_bar):
        ...
        if len(self.data.Close) < 100:  # Ensure enough data for indicators
            return
        ...
        if cond1 and cond2 and cond3:
            ...
            if stop_distance > 0:
                ...
                if position_size > 0:
                    ...
                    print(f"[BUY] Position size: {position_size}, Entry: {entry_price:.2f}, Stop: {stop_price:.2f}")
    ...
    def manage_position(self, current_bar):
        ...
        candles_since_entry = current_bar - self.position.entry_bar
        
        # Check exit conditions
        
        # 1. Check for TEMA cross above in any exit pair
        exit_signal = False
        for i, (short_tema, long_tema) in enumerate(self.exit_tema_pairs):
        ...
        if exit_signal:
            self.position.close()
            print(f"[EXIT] Closed position at {current_price:.2f}, PnL: {self.position.pl:.2f}")
            return
        ...
        if current_profit_pct >= target_profit and target_profit > 0:
            ...
            print(f"[EXIT] Closed at {current_price:.2f}, PnL: {self.position.pl:.2f}")
            return
