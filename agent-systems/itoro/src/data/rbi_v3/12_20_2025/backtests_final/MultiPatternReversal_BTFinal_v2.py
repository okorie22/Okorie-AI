...
class MultiPatternReversal(Strategy):
    ...
    def next(self):
        ...
        if entry_signal:
            ...
            # CRITICAL FIX: Actually execute the buy order
            # This was missing or not executing properly
            try:
                self.buy(size=position_size)
                print(f"[DEBUG] BUY ORDER EXECUTED: size={position_size}")
                self.trade_count += 1
                ...
                print(f"[DEBUG] Equity: {self.equity}, Position size: {position_size}")
        else:
...
# CRITICAL FIX: Ensure we have the required columns in the correct format
# Drop any extra columns that might interfere
