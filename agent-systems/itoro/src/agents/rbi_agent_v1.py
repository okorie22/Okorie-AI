...
# Import RBI configurations from config.py
from src.config import (
    ...
    RBI_DATA_DIR,
    RBI_IDEAS_FILE,
    RBI_RBI_PROCESSED_IDEAS_LOG,
    RBI_TODAY_DIR,
    RBI_RBI_RESEARCH_DIR,
    RBI_RBI_BACKTEST_DIR,
    RBI_RBI_PACKAGE_DIR,
    RBI_FINAL_RBI_BACKTEST_DIR,
    RBI_CHARTS_DIR,
    RBI_EXECUTION_DIR,
    RBI_CONDA_ENV,
    RBI_MAX_DEBUG_ITERATIONS,
    RBI_EXECUTION_TIMEOUT,
    RBI_AI_TEMPERATURE,
    RBI_AI_MAX_TOKENS,
    RBI_DATA_RETENTION_DAYS,
    RBI_MAX_IDEAS_PER_RUN,
    RBI_ENABLE_CLOUD_STORAGE,
    RBI_CLOUD_TABLE_NAME,
    RBI_DEFAULT_POSITION_SIZE,
    RBI_DEFAULT_CASH,
    RBI_ENABLE_YOUTUBE_TRANSCRIPTS,
    RBI_ENABLE_PDF_EXTRACTION,
    RBI_MAX_CONTENT_LENGTH
)

# Use RBI config variables
RESEARCH_CONFIG = RBI_RESEARCH_CONFIG
BACKTEST_CONFIG = RBI_BACKTEST_CONFIG
DEBUG_CONFIG = RBI_DEBUG_CONFIG
PACKAGE_CONFIG = RBI_PACKAGE_CONFIG
...
# Import cloud database functions for RBI
try:
    from src.scripts.database.cloud_database import save_rbi_strategy_result
    CLOUD_DB_AVAILABLE = True
except ImportError:
    CLOUD_DB_AVAILABLE = False
    cprint("⚠️ Cloud database not available for RBI results", "yellow")

# Use RBI configuration variables from config.py
# All directory paths and settings are now imported from config.py

# Create main directories if they don't exist
for directory in [RBI_DATA_DIR, RBI_TODAY_DIR, RBI_RESEARCH_DIR, RBI_BACKTEST_DIR,
                  RBI_PACKAGE_DIR, RBI_FINAL_BACKTEST_DIR, RBI_CHARTS_DIR, RBI_EXECUTION_DIR]:
    directory.mkdir(parents=True, exist_ok=True)

# Clean up old data based on retention policy
def cleanup_old_rbi_data():
    """Clean up old RBI data directories based on retention policy"""
    try:
        if not RBI_DATA_DIR.exists():
            return

        from datetime import datetime, timedelta
        cutoff_date = datetime.now() - timedelta(days=RBI_DATA_RETENTION_DAYS)

        cleaned_count = 0
        total_space_saved = 0

        for item in RBI_DATA_DIR.iterdir():
            if item.is_dir() and item.name != RBI_TODAY_DIR.name:
                try:
                    # Try to parse directory name as date (MM_DD_YYYY format)
                    dir_date = datetime.strptime(item.name, "%m_%d_%Y")
                    if dir_date < cutoff_date:
                        # Calculate space usage before deletion
                        space_used = sum(f.stat().st_size for f in item.rglob('*') if f.is_file())

                        import shutil
                        shutil.rmtree(item)
                        cleaned_count += 1
                        total_space_saved += space_used
                        cprint(f"🗑️ Cleaned up old RBI data: {item.name} ({space_used/1024/1024:.1f}MB)", "blue")
                except ValueError:
                    # Not a date directory, skip
                    continue

        if cleaned_count > 0:
            cprint(f"✅ Cleaned up {cleaned_count} old RBI data directories, saved {total_space_saved/1024/1024:.1f}MB", "green")

        # Also clean up processed ideas log if it gets too large
        if RBI_PROCESSED_IDEAS_LOG.exists():
            try:
                # Keep only the last 1000 entries to prevent log from growing too large
                with open(RBI_PROCESSED_IDEAS_LOG, 'r') as f:
                    lines = f.readlines()
                ...
                cprint(f"⚠️ Error trimming processed ideas log: {e}", "yellow")

    except Exception as e:
        cprint(f"⚠️ Error during RBI data cleanup: {e}", "yellow")

# Run cleanup on startup
cleanup_old_rbi_data()

cprint(f"📂 Using RBI data directory: {RBI_DATA_DIR}")
cprint(f"📅 Today's date folder: {RBI_TODAY_DIR.name}")
cprint(f"📂 Research directory: {RBI_RESEARCH_DIR}")
cprint(f"📂 Backtest directory: {RBI_BACKTEST_DIR}")
cprint(f"📂 Package directory: {RBI_PACKAGE_DIR}")
cprint(f"📂 Final backtest directory: {RBI_FINAL_BACKTEST_DIR}")
cprint(f"📈 Charts directory: {RBI_CHARTS_DIR}")
cprint(f"⚡ Execution directory: {RBI_EXECUTION_DIR}")
    ...
    try:
        ...
        if model_config["type"] == "ollama":
            response = model.generate_response(
                ...
                temperature=RBI_AI_TEMPERATURE
            )
...
def process_trading_idea(idea: str) -> None:
    ...
    print(f"📅 Saving results to today's folder: {RBI_TODAY_DIR.name}")
    ...
    try:
        ...
        # Save research output
        research_file = RBI_RESEARCH_DIR / f"{strategy_name}_strategy.txt"
        ...
        backtest_file = RBI_BACKTEST_DIR / f"{strategy_name}_BT.py"
        ...
        package_file = RBI_PACKAGE_DIR / f"{strategy_name}_PKG.py"
        ...
        # Save to cloud database if enabled
        if RBI_ENABLE_CLOUD_STORAGE and CLOUD_DB_AVAILABLE:
            try:
                print("\n☁️ Saving strategy results to cloud database...")

                # Prepare strategy data for cloud storage
                strategy_data = {
                    'strategy_name': strategy_name,
                    'strategy_type': 'youtube' if 'youtube.com' in idea or 'youtu.be' in idea else 'pdf' if idea.endswith('.pdf') else 'text',
                    'source_url': idea if idea.startswith('http') else None,
                    'research_content': strategy,
                    'backtest_code': final_backtest,
                    'execution_status': 'success',
                    'ai_model_used': f"{RESEARCH_CONFIG['type']}-{RESEARCH_CONFIG['name']}",
                    'processing_time_seconds': int(time.time() - start_time),
                    'metadata': {
                        'research_file': str(research_file.relative_to(RBI_DATA_DIR)) if research_file.exists() else None,
                        'backtest_file': str(backtest_file.relative_to(RBI_DATA_DIR)) if backtest_file.exists() else None,
                        'package_file': str(package_file.relative_to(RBI_DATA_DIR)) if package_file.exists() else None,
                        'final_file': str(final_file.relative_to(RBI_DATA_DIR)) if final_file.exists() else None
                    }
                }

                if save_rbi_strategy_result(strategy_data):
                    print("✅ Strategy results saved to cloud database!")
                else:
                    print("⚠️ Failed to save to cloud database, but local files are safe")

            except Exception as cloud_error:
                print(f"⚠️ Cloud database save failed: {cloud_error}")
                print("📁 Local files are still available")

    except Exception as e:
        print(f"\n❌ Error processing idea: {str(e)}")

        # Log failed strategy to cloud if enabled
        if RBI_ENABLE_CLOUD_STORAGE and CLOUD_DB_AVAILABLE:
            try:
                failed_data = {
                    'strategy_name': 'error_processing',
                    'strategy_type': 'error',
                    'source_url': idea if idea.startswith('http') else None,
                ...
                save_rbi_strategy_result(failed_data)
            except:
                pass  # Don't let cloud logging failures crash the main error
...
if __name__ == "__main__":
    try:
        ...
        cprint(f"📅 Today's Date: {RBI_TODAY_DIR.name} - All outputs will be saved in this folder", "magenta")
        cprint(f"🧠 DeepSeek-R1 thinking tags will be automatically removed from outputs", "magenta")
        cprint(f"📋 Processed ideas log: {RBI_PROCESSED_IDEAS_LOG}", "magenta")
