...
# Matplotlib and mplfinance are required for chart generation
try:
    import matplotlib
    ...
    raise ImportError(
        "matplotlib and mplfinance are required for chart analysis. "
        "Please install them with: pip install matplotlib mplfinance"
    ) from e
...
from scripts.shared_services.logger import debug, info, warning, error, critical, system
from scripts.shared_services.shared_api_manager import get_shared_api_manager
from scripts.shared_services.shared_data_coordinator import get_shared_data_coordinator
# Trade lock manager removed - now using SimpleAgentCoordinator

# Cloud database import
try:
    from scripts.database.cloud_database import get_cloud_database_manager
...
class ChartAnalysisAgent(BaseAgent):
    def __init__(self):
        ...
        self.running = True
        self.token_map = getattr(config, 'TOKEN_MAP', {})
        ...
        # Set up directories
        self.charts_dir = PROJECT_ROOT / "data" / "charts"
        ...
        # Set AI parameters - use config values with fallbacks
        self.ai_model = getattr(config, 'AI_MODEL', 'claude-3-5-sonnet-20241022')
        self.ai_temperature = getattr(config, 'AI_TEMPERATURE', 0.7)
        self.ai_max_tokens = getattr(config, 'AI_MAX_TOKENS', 4000)
    ...
    def _analyze_chart(self, symbol, timeframe, data):
        ...
        try:
            ...
            filepath = CHARTS_DIR / f'chart_analysis_{symbol}.csv'
            ...
            if analysis and 'action' in analysis:
                ...
                prev_filepath = CHARTS_DIR / f'chart_analysis_{symbol}.csv'
    ...
    def _save_chart_sentiment_to_csv(self, sentiment_data):
        ...
        try:
            ...
            filepath = CHARTS_DIR / AGGREGATED_SENTIMENT_FILE
