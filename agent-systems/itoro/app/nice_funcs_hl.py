...
try:
    ...
    from ta_indicators import ta  # For technical indicators
import traceback
try:
...
def hyperliquid_perp_entry(symbol: str, side: str, size_usd: float, leverage: int = 5, reduce_only: bool = False):
    ...
    Returns:
        dict: Order response
    """
    from config import HYPERLIQUID_WALLET_ADDRESS, HYPERLIQUID_PRIVATE_KEY, HYPERLIQUID_TESTNET
...
def hyperliquid_leverage_entry(token_address: str, direction: str, confidence: float, usd_size: float):
    ...
    Returns:
        dict: Trade result
    """
    from config import DEFAULT_LEVERAGE, LEVERAGE_SUPPORTED_ASSETS
