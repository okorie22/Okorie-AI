...
from config import EDITING_CONFIG
...
class EditingAgent:
    ...
    def __init__(self, pipeline_manager, config_override: Optional[Dict[str, Any]] = None):
        ...
        config = EDITING_CONFIG.copy()
        if config_override:
            config.update(config_override)

        self.pipeline_manager = pipeline_manager
        self.output_format = config["output_format"]
        self.add_captions = config["add_captions"]
        self.background_music = config["background_music"]
        self.target_resolution = config["target_resolution"]
        self.auto_color_correction = config["auto_color_correction"]
        self.denoise = config["denoise"]
        self.sharpen = config["sharpen"]
        self.stabilize = config["stabilize"]

        # Font settings
        self.caption_font = config["caption_font"]
        self.caption_fontsize = config["caption_fontsize"]
        self.caption_color = config["caption_color"]
        self.caption_box_color = config["caption_box_color"]

        # Output directories
        self.edited_clips_dir = Path(config["edited_clips_dir"])
        self.music_library_dir = Path(config["music_library_dir"])
        self.edited_clips_dir.mkdir(parents=True, exist_ok=True)
        self.music_library_dir.mkdir(parents=True, exist_ok=True)

        # Resolution settings
        if self.target_resolution == "4k":
            self.width = 2160
            self.height = 3840
        else:  # 1080p
            self.width = 1080
        ...
        logger.info(f"  Format: {self.output_format} ({self.width}x{self.height})")
        logger.info(f"  Captions: {self.add_captions}")
        logger.info(f"  Background music: {self.background_music}")
        logger.info(f"  Color correction: {self.auto_color_correction}")
        logger.info(f"  Denoise: {self.denoise}, Sharpen: {self.sharpen}")
