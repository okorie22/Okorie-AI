...
class ClipsAgent:
    ...
    def __init__(self, deepseek_connection, pipeline_manager,
                 target_duration_min: int = 15, target_duration_max: int = 60,
                 num_clips: int = 3):
        """
        ...
        Args:
            ...
            target_duration_min: Minimum clip duration in seconds
            target_duration_max: Maximum clip duration in seconds
            num_clips: Number of clip candidates to generate
        """
        ...
        logger.info(f"  Target duration: {target_duration_min}-{target_duration_max}s")
        logger.info(f"  Clips per video: {num_clips}")
...
Provide exactly {self.num_clips} clip candidates, ordered by score (highest first)."""
    ...
    def _create_fallback_clips(self, duration: float) -> List[Dict[str, Any]]:
        ...
        clips = []
        clip_duration = min(self.target_duration_max, duration / self.num_clips)
        
        for i in range(min(self.num_clips, int(duration / clip_duration))):
